...
How a single species phenological cues varies across space is not yet well predicted. Numerous studies have investigated how phenological cues for budburst vary across space, including variation across populations, by using latitudinal gradients \citep{Partanen2004, Viheraaarnio2006, Caffarra2011, Zohner2016, Gauzere2017} [BUT THEY FIND VARYING PATTERNS?]. Fewer, however, have integrated distance from the coast \citep [but see][]{Myking2007, Harrington2015, Aitken2015} or regional effects. Some studies indicate that populations further inland will initiate budburst first, whereas those closer to the coast will initiate budburst later in the season and that the distance from the coast is a stronger indicator of budburst timing than latitude (REFS).
...
methodplot<-ggplot(blend, aes(year, FSI)) + xlab("Year") +
  ...
  geom_line(aes(x=year, y=FSI, col=Method)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        ...
        axis.line = element_line(colour = "black"), legend.key=element_blank())  + geom_hline(yintercept=0, linetype="dotted") + geom_hline(yintercept=7, linetype="dotted") + 
  scale_shape_manual(labels = c("SI-x", "O`Keefe", "Phenocam"),
                     values = c(16, 17, 15)) + ggtitle("Comparison of Calculated FSI Values")
plot(methodplot)
...
temp<-temp +coord_flip() + ggtitle("Damaging Temperature Thresholds")
plot(temp)
@
