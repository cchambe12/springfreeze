...
Temperate forest plants experience elevated risk of frost damage during the spring due to the stochastic timing of frosts. Freezing temperatures following a warm spell can result in plant damage or even death \citep{Ludlum1968, Mock2007}. Many temperate species exhibit flexible spring phenologies, which help them minimize spring freezing risk, but freeze damage can still occur. Once buds exit the dormancy phase, they are less freeze tolerant and less resistant to ice formation \citep{Taschler2004, Lenz2013, Vitasse2014a}. %CJC (3-Jul-2018) Intracellular ice formation from false spring events often results in severe leaf and stem damage \citep{Burke1976, Sakai1987}. Ice formation can also occur indirectly (i.e. extracellularly), which results in freezing dehydration and mimics drought conditions \citep{Pearce2001, Beck2004, Hofmann2015}. Both forms of ice formation can cause defoliation and crown dieback \citep{Gu2008}. 
...
{Chuine2010} probably play a dominant role. Most phenological studies currently focus on one phenophase (i.e. budburst or leafout) but, in order to examine false spring risk, it is crucial to examine the effects of the three phenological cues and their interactions on the duration of vegetative risk (i.e. both budburst and leafout). Thus, to advance our understanding of false springs, studies should measure the response to the three major cues and to also focus on the single versus interactive effects of those cues on multiple phenophases. 
...
One study examined how these cues impact budburst and leafout. The experiment examined 9 temperate trees and shrubs using a fully crossed design of three levels of chilling (field chilling, field chilling plus 30 days at either 1.5 or 4 $^{\circ}$C), two levels of forcing (20$^{\circ}$C/10$^{\circ}$C or 15$^{\circ}$C/5$^{\circ}$C day/night temperatures) and two levels of photoperiod (8 versus 12 hour days) resulting in 12 treatment combinations. Using an example from a recent study that manipulated the three cues and measured budburst and leafout, we compared treatment conditions that  for plants (i.e. before climate change) and compared those durations of vegetative risk to those under climate change conditions.

% And then just mention how we compared the two extreme treatments to begin to predict the effects of climate change on the duration of vegetative risk, which will tie in how different species respond differently to the three cues (e.g. Betula alleghaniensis) and lead us in to the next section on regional differences. 

% After that I think you want to hit two big points: people need to measure the response to cues and think about their single versus interactive effects, if you want to do prediction.

One study, which examined how these cues impact budburst and leafout, shows that the duration of vegetative risk can vary by 21 days or more depending on the suite of cues a plant experiences (Figure \ref{fig:dan}) \citep{Flynn2018}. The experiment examined 9 temperate trees and shrubs using a fully crossed design of three levels of chilling (field chilling, field chilling plus 30 days at either 1 or 4 $^{\circ}$C), two levels of forcing (20$^{\circ}$C/10$^{\circ}$C or 15$^{\circ}$C/5$^{\circ}$C day/night temperatures) and two levels of photoperiod (8 versus 12 hour days) resulting in 12 treatment combinations. Increased forcing, photoperiod and chilling all decreased the duration of vegetative risk, with forcing causing the greatest decrease (10 days), followed by daylength (9 days), and chilling (2-3 days depending on the temperature), but the full effect of any one cue depended on the other cues due to important interactions---for example, the combined effect of warmer temperatures and longer days would be 14 days, because of -5 days interaction between the forcing and photoperiod cues (Figure \ref{fig:dan}A). 
...
methodplot<-ggplot(blend, aes(year, FSI)) + xlab("Year") +
  ...
  geom_point(aes(col=Method, shape=Method)) + scale_color_manual(values=c("#F8766D","#00C094","#00B6EB","#A58AFF"), labels=c("SI-x", "O`Keefe", "Phenocam")) +
  geom_line(aes(x=year, y=FSI, col=Method)) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), 
        panel.background = element_blank(), 
        axis.line = element_line(colour = "black"), legend.key=element_blank(), plot.title = element_text(family="Helvetica"))  + geom_hline(yintercept=0, linetype="dotted") + geom_hline(yintercept=7, linetype="dotted") + 
  scale_shape_manual(labels = c("SI-x", "Observational (O`Keefe)", "Phenocam"),
                     values = c(16, 17, 15)) + ggtitle("Calculating False Spring Risk")
plot(methodplot)

@

<<label=temp, results="asis", echo=FALSE, warning=FALSE, message=FALSE, fig.cap="A comparison of damaging spring freezing temperature thresholds across ecological and agronomic studies. Each study is listed on the y axis along with the taxonomic group of focus. Next to the species name is the freezing definition used within that study (e.g. 100\\% is 100\\% lethality). Each point is the best estimate recorded for the temperature threshold with standard deviation if indicated in the study.", fig.pos="H", fig.width=6, fig.height=4, fig.align='center'>>=
...
\begin{figure} [H] 
 \begin{center}
 %\textbf{Effects of Cue Interactions on the Duration of Vegetative Risk}\par\medskip
 \includegraphics[width=16cm, height=10cm]{..//figure/DVR_diffplots_nophoto.pdf} 
 \caption{How major cues of spring phenology alter vegetative risk. (A) Model estimates of changes in the duration of vegetative risk due to varying cues from a full-factorial experiment (means $\pm$ 95\% credible intervals, slightly larger blue circles represent the overall mean estimate, while each species estimate is shown below and colored as shown in B). Higher forcing temperatures decreased the period of vegetative risk the most (by 10 days overall given a 10 degree difference), as did photoperiod (by 9 days overall given a 4 hour increase). However, together these effects offset, thus the combined effect of greater forcing and longer photoperiod would be a reduction in duration of vegetative risk of 14 days due to a 5 day delay through their interaction. (B) A comparison of the durations of vegetative risk (raw data means $\pm$ 1 standard error) across two treatments (high chilling and high forcing temperatures vs. low chilling and low forcing, in both photoperiod was 12 hr) for each species in the experiment. Species along the x-axis are ordered by day of budburst.}\label{fig:dan} 
 \end{center}
 \end{figure}
