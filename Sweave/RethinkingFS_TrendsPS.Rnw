...
Such cues may provide a starting point for predicting how climate change will alter the duration of vegetative risk. Robust predictions will require much more information, especially the emissions scenario realized over coming decades \citep{IPCC2014}, but one potential outcome is that higher temperatures will increase forcing and decrease chilling in many locations. We used data from a recent study that manipulated the three cues and measured budburst and leafout \citep{Flynn2018}. We then we calculated the effects of the three major cues and their interactions on the duration of vegative risk using a classical experimental approach (Figure \ref{fig:dan}A) (i.e. with increased forcing and increased photoperiod, the duration of vegetative risk decreases). But in order to assess the effects of climate change, we should look at the effects of increased forcing, decreased photoperiod and decreased chilling (Figure \ref{fig:dan}A). Under this scenario, experimental results suggest a 2-5 day increase in duration of vegetative risk with increased forcing but decreased chilling and a 7-10 day increase in duration of vegetative risk with increased forcing and decreased photoperiod. These cue interactions could thus expose at-risk plants to more intense false spring events or even multiple events in one year.
...
\begin{figure} [H] 
 \begin{center}
 \textbf{Effects of Climate Change on the Duration of Vegetative Risk}\par\medskip
 \includegraphics[width=16cm, height=10cm]{..//figure/Exp_plot_Comparison.pdf} 
 \caption{How major cues of spring phenology alter vegetative risk. Panel (A) demonstrates the more classical approach to assessing phenological cues and their interactions, which indicates fewer days between budburst and leafout with increasing forcing, chilling, and photoperiod. However, in climate change studies, this not realistic nor predicted. In panel (B) we demonstrate the predicted shifts in climate by showing the duration in vegatitive risk with increased forcing, decreased chilling and shorter photoperiods and their interactions. To calculate the interation, for example, we added the effect of forcing, photoperiod and the interaction effect of forcing and photoperiod for each species.   }\label{fig:dan} 
 \end{center}
 \end{figure}
