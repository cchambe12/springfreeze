...
<<label=hf, results="asis", echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Duration of vegetative risk across species for 1997 and 2012. In 1997, the aggregated GDDs to budburst were the lowest and the duration of vegetative risk for each species are short, whereas in 2012, the aggregated GDDs to budburst were the highest and the duration of vegetative risk is higher. The dotted line indicates a False Spring event in 2012. The histogram at the bottom right corner indicates the frequency of accumulated GDDs to budburst for each year and indicates that 1997 is the lowest and 2012 is the highest. ", fig.pos="H", fig.width=6, fig.height=4, fig.align='center'>>=
...
hist<-ggplot(bbg, aes(x=m.bb)) + geom_histogram(aes(fill=colors), binwidth = 20, color="gray30", size=0.3) +
  ...
  theme(panel.background = element_blank(), axis.line = element_line(colour = "black"), 
        legend.position = c(0.05,0.85), legend.text = element_text(size=8), legend.key.size = unit(0.05, "cm"),
        axis.title=element_text(size=10), legend.title = element_text(size=8), axis.text=element_text(size=8))

vp <- viewport(width = 0.3, height = 0.4, x = 0.90,
