...
\item Already, multiple studies have documented false spring events in recent years \citep{Gu2008, Augspurger2009, Knudson2012, Augspurger2013} and some have linked this to climate change \citep{Ault2013, Allstadt2015, Muffler2016, Xin2016}. So, there's growing interest in false spring has led to a lot of research investigating the effects on temperate forests and agricultural crops. 
...
\item First, we review the currently used definitions of false spring.
...
\begin{enumerate}
\item Several current definitions, such as ... 
...
\end{enumerate}
\item One widely used definition ... 
...
\begin{enumerate}
\item We applied this definition to one forest (Harvard Forest)
\item These three methodologies were compared using long-term ground observational data \citep{Okeefe2014}, PhenoCam data from Harvard Forest \citep{Richardson2015}, and USA-NPN SI-x \citep{USA-NPN2016} and then inputted into the FSI equation (Equation \ref{eq:1}) to calculate FSI values from 2008 to 2014 (Figure \ref{fig:fsifig}).
\item But each gives a slightly different answer ... why, because each represents a different group of species, and currently most FSI work ignores such variation, but -- given how the equation is built half on spring onset date, it's critical (current false spring equation only uses two inputs: date of spring onset and date of last freeze (Equation \ref{eq:1})). 
\end{enumerate}
...
\begin{enumerate}
\item Elucidate the differences between spring onset dates and functional groups
\begin{enumerate}
...
\item Connect Figure 1 here to differences in functional groups:
\begin{enumerate}
...
\end{enumerate}
\item Therefore, one spring onset date cannot be used as an effective proxy for all species. % One point that I would like to make over the course of the paper is that these smaller species are opportunistic now and are willing to take the false spring risk, but with spring advancing and last freeze date staying the same, these little guys are no longer taking that risk, the big guns are, which could be really shocking to the canopy folks and potentially severely detrimental to the entire system.... EMW says: Yes, that's super cool, a bit speculative though so might be something to try to sneak into paper once towards the end, after you have convinced readers of your other points. 
...
\item Transition sentence (along the lines of): But considering different functional groups is unlikely to be enough for robust predictions.
% Above is similar to: 
% False spring is defined dependent on metholodology for onset, make sure your methodology matches your question.
% And: A suitable methodology for determining spring onset is crucial in order to establish an effective model for false spring risk.
\end{enumerate}
...
\item Considering the different avoidance and tolerance strategies commonly seen in temperate forest species is critical 
...
\section*{Plant physiology and diversity versus current FS definition} % Nice!
\begin{enumerate}
...
{\bf Insert transition here...} something like ... given all this understanding/predicting when plants will actually be damaged by false spring events requires much more nuance.
...
\section*{Defining Vegetative Risk: Complexities due to species' strategies and climate change} % This section maybe needs species differences somewhere else? 
...
\begin{enumerate}
\item Species differences in their responses to climate change: They matter 
\begin {enumerate}
...
\begin{enumerate}
\item Tell the forcing temperature story % Nice!
\begin{enumerate}
...
\begin{enumerate} % Probably will only be able to fit in < half of below points, decide which are most important to you! I think this could be shortened a lot and combined with next section (drive home point of new equation)
...
% Stuff I think we now say elsewhere:
...
% Great points but I don't the paper has room for them:
