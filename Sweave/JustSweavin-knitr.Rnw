...
\begin{document}
\bibliography{~/Documents/git/falsespring/library.bib}
...
<<label=sourcestuff, echo=FALSE, results="hide",message=FALSE>>=
...
<<label=hf, results="asis", message=FALSE, echo=FALSE, fig.cap="A scatterplot indicating FSI values from 2008 to 2014 for PhenoCam and Observed data. PhenoCam FSI values are red and Observed FSI values are blue.", fig.align='center'>>=
plot(hf)
@
