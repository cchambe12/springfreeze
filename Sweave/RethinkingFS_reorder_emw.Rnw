...
hf.bb<-ggplot(gdd.yr, aes(x=ord,ymin = bb.jd, ymax = l75.jd, group=interaction(species, year) )) +
  ...
  guides(color = guide_legend(override.aes = list(linetype = c(1, 1, 0, 0)))) + scale_x_discrete(labels=c(
    expression(italic("Quercus rubra")), expression(italic("Q. alba")), expression(italic("Hamamelis virginiana")), expression(italic("Fraxinus americana")),
    expression(italic("Betula alleghaniensis")), expression(italic("Acer saccharum")), expression(italic("A. rubrum")), expression(italic("A. pensylvanicum")))
  )
