...
\item These serations could permit greater packability into winter buds, which reduces the metabolic requirements in the spring and increasing the rate of leaf out \citep{Edwards2017}.
\item Young temperate forest plant leaves also generally have more trichomes (i.e. small, unicellular hairs), giving leaves greater protection against false springs once budburst has already initiated \citep{Agrawal2004}.
