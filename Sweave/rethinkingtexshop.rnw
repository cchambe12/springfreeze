...
False spring events also put seedling and sapling trees at greater risk to damage than adult trees \citep{Vitasse2014}. Younger trees are more likely to incur lastly damage to the leaf buds and vegetative growth, whereas adult trees are at risk of xylem embolism %need a reference here and to elaborate. Xylem embolism could have greater impact but is less likely to happen. It would require more intensive freezing events - longer and colder than just damage to seedling and sapling leaf buds. 
...
<<label=fsifig, results="asis", echo=FALSE, fig.cap="A scatterplot indicating FSI values from 2008 to 2014 for each methdology used in this study. PhenoCam FSI values are red, Observed FSI values are blue, and USA-NPN FSI values are green.", fig.pos="H">>=
plot(methodplot)
@
