...
\bibliography{/Users/CatherineChamberlain/Documents/git/springfreeze/refs/SpringFreeze.bib}
\bibliography{SpringFreeze.bib}

\begin{document}
%\bibliographystyle{/Users/CatherineChamberlain/Documents/git/springfreeze/refs/SpringFreeze.bib} 
...
More frequently, however, plants that have been exposed to a false spring will experience leaf loss and slower canopy development %\cite{Hufkens2012}. 
...
<<label=bb.table, echo=FALSE, results="asis">>=
...
<<label=fsitable, results="asis", echo=FALSE>>=
...
<<label=fsifig, results="asis", echo=FALSE, fig.cap="A scatterplot indicating FSI values from 2008 to 2014 for each methdology used in this study. PhenoCam FSI values are red, Observed FSI values are blue, and USA-NPN FSI values are green.">>=
plot(methodplot)
@
...
<<label=pearson, results="asis", echo=FALSE>>=
