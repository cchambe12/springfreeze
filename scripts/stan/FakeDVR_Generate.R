...
(fake <- data_frame(risk, warm, photo, chill1, chill2))

summary(lm(risk ~ (warm+photo+chill1+chill2)^2, data = fake)) # sanity check 
...
for(i in 1:nsp){ # loop over species, as these are the random effect modeled
  ...
  fakex <- data.frame(risk, sp = i, warm, photo, chill1, chill2)
...
summary(lm(risk ~ (warm+photo+chill1+chill2)^2, data = fake)) # double sanity check 
